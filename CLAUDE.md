...
클라이언트 ID에 더해 프록시 설정·앱 토큰도 비우며(2026-09-24 t10 — 카카오·ODsay 조회가
프록시를 타지 못하게 구조적으로 끊는다), 내부 기한(기본 900초, 명령줄 인자 하나로 덮을 수
있음)이 지나면 스스로 끝난다. 실제 지원 디렉터리는 시작·종료에 읽어서 바이트로 대조만 한다.
...
정상 종료 분은 샌드박스(`$TMPDIR/besir-gd-<UUID>`)를 스스로 지우지만, 시한 종료(124)·강제
종료가 남긴 것은 수동으로 정리한다. 자동 청소기(스위퍼)는 두지 않기로 했다(2026-09-24) —
남은 게 보이면 `besir-gd-` + UUID 모양인 것만 확인해 지운다: `ls -d $TMPDIR/besir-gd-*`
