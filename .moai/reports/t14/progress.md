...
코드 커밋 **`96529a4`**, 문서 커밋은 본 보고서 포함 아래.

## §1 주장 (Claim)

1. **계약 6 위반 전수 정리(11파일 +41/−35)** — 디스패치 노트대로 day-close 좌표를 현재 트리에서
   재실측해 확정했다(CB 3건은 `AddEventView :732·:735·:740` — t13 수리에서 CHECKLIST에 재사상된
   좌표와 일치). 매핑: `.secondary`→`Theme.muted` 17곳, `.tertiary`→`Theme.faint` 3곳,
   직접 색 5곳 — AIChatView 말풍선 `.white`/`.primary`→`Theme.bg`/`Theme.ink`(다크 대비
   ≈2.5:1→≈7.3:1, code-safety 16진수 손계산), FavoritesView 별 `.yellow`→`Theme.activity`(브라스),
   SettingsView 연결됨 `.green`→`Theme.travel`. 카드 좌표 밖 같은 부류(FavoritesView·FullSirView·
   SettingsView·AIChatView:85)도 디스패치 계약 문구 "색은 전부 Theme 토큰 경유"에 따라 함께 정리했다.
...
- code-safety의 대비비 ≈7.3:1은 16진수 손계산 근사(도구 실측 아님).
- `ShareExtension/ShareViewController.swift:19` cornerRadius 14(UIKit layer) — 기존·카드 범위
  밖, 미수리(전체 앱 "각진 면" 문법의 마지막 미문서화 literal — 후보).
- 아이콘 전용 버튼 무라벨 부류의 추가 회원(FavoritesView:36 trash·FullSirView:144 돋보기·
  :293 trash·FullSirView:60·AIChatView:63 `.help`만 — iOS VoiceOver 불능) — 이 카드는 카드가
  지목한 1건만 수리. **별도 a11y 스윕 카드 후보**(등록은 리드 소관).
...
:161·:240·:294·:317, EditCardView:31-35·:50-54·:125-128·:145-152·:175-180·:186-263, AIAssistant
