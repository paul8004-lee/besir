...
- 수리 커밋: **`7d15e94`**("fix(guard): t10 sync 수리 — D1 관측 재귀속·D2 좌표 + N1-N4 (t10)") —
  `ec4fc31`·`98204c6` 위의 이 브랜치 세 번째 커밋. 본 §11 기입 커밋은 SHA 기록만 담는다.
- 표집기 #2 최종 확정(§10 재실측의 관측 근거): 루프 종료 후 `/tmp/gd-t10-fix2-net.log` **0줄** —
  전 구간 커버 표집의 소켓 관측 0건이 확정됐다(커밋 시점에 관측 완료 상태로 기록).
