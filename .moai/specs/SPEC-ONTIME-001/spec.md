...
| 0.2.4 | 2026-09-15 | SPEC-ASK-001(얕은 메모리 제거) REQ-030/031이 요구한 as-built 반영 — 커밋 `7683700`(M1~M3)으로 `remember_fact`·`forget_fact` 툴과 `ai_memory.json` 경로가 사라졌다. REQ-061을 "정확히 11개 툴"에서 9개로 개정(선언 `toolsJSON()` `:828`~`:963` 실측), REQ-063에서 `ai_history.json` 외 지속과 remember/forget 경로 삭제(대화 이력 지속·실패 턴 롤백은 유지). acceptance.md AC-007의 "11개 툴"·기억 저장/삭제·`ai_memory.json` 서술을 동조하고 밀린 근거 인용 재정렬(`zeroUpdateIssue` `:1500`, `series_number` 해석 `:1436`·`:1443`), plan.md M5 요약 갱신. REQ-062 본문의 인용 줄번호 밀림은 이 개정 범위 밖 |
...
- **REQ-061 (Ubiquitous)**: The `AIAssistant` tool loop shall expose exactly these 9 tools: `create_schedule`, `create_activity`, `create_recurring_schedule`, `update_recurring_schedule`, `update_schedule`, `list_schedules`, `recommend_meal`, `check_travel_time`, `delete_schedule`. (2026-09-15 개정, SPEC-ASK-001 REQ-030 — 얕은 메모리 툴 `remember_fact`·`forget_fact`가 커밋 `7683700`에서 제거되며 11개 → 9개가 됐다. 선언은 `toolsJSON()` `:828`~`:963`에 있고, GuardDriver 단언 "(e) 툴은 9개다(remember_fact·forget_fact 제거)"가 HEAD `b9ef208`의 88/88 실행에서 통과했다 — SPEC-ASK-001 progress.md §E.2.)
...
- **REQ-063 (Ubiquitous)**: The `AIAssistant` shall persist conversation history to `ai_history.json` (경로 조립 `:175`), surviving app relaunch and reinstall; **when** a tool-call turn fails, it shall roll history back to its pre-turn state rather than persisting a broken turn that would repeat the same failure on every subsequent request. (2026-09-15 개정, SPEC-ASK-001 REQ-031 — 장기 선호 지속 `ai_memory.json`과 `remember_fact`/`forget_fact` 경로를 커밋 `7683700`에서 제거했다. 앱은 `ai_memory.json`을 더 이상 읽지도 쓰지도 않으므로 기기에 남은 옛 파일은 동작에 영향을 주지 않는다 — 삭제 마이그레이션은 요구하지 않는다.)
