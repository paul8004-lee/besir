...
| AC | 상태 |
|---|---|
| AC-201 | ✅ PASS (2026-09-12, 실기기 확인 — 정상 동작. 사용자 의견으로 위치를 활동 상세로 이전, 아래 "추가 반영" 참고) |
| AC-202 | ✅ PASS (2026-09-12, 실기기 확인) |
| AC-203 | ✅ PASS (2026-09-12, 실기기 확인 — 할루시네이션 없음) |
| AC-204 | ⚠️→✅ FAIL 발견 후 수정 완료 (2026-09-12) — 아래 근거 |
| AC-205 | ⬜ 미실행 (연결된 버그 AC-204에서 발견·수정됐으나 삭제 시나리오 자체는 아직 실기기 재확인 전) |
| AC-206 | ✅ PASS (2026-09-12, 실기기 확인) |
| AC-207 | ✅ PASS (2026-09-12, 실기기 확인) |
...
**실기기 결과(2026-09-12)**: 지연 조회 자체는 정상 동작. 다만 사용자가 "식사하는 곳"인 활동 일정 쪽에 있는 게 "이동하는 중"인 이동 일정보다 자연스럽다는 의견을 줘, **섹션을 `EventDetailView`에서 `ActivityDetailView`로 옮겼다**(REQ-006이 서술하는 지연조회 동작 자체는 그대로 유지, 위치만 이전 — REQ 본문 수정은 spec.md HISTORY에 별도 기록). **PASS**.
...
**실기기 결과(2026-09-12)**: 두 기준 위치 전환 정상 동작 확인("바뀜"). **PASS**. **추가 반영**: 사용자가 "외곽 지역처럼 현재 위치·오늘 일정 목적지 둘 다 아닌 곳을 기준으로 검색할 방법이 없다"고 지적 — 이 AC의 범위 밖(새 요구사항)이라 `Basis.custom` 케이스 + `PlaceField` 기반 직접 검색을 추가했다(아래 "추가 반영" 참고).
...
**실기기 결과(2026-09-12)**: "추천 가게 실제 검색 결과에 있음" — 할루시네이션 없음 확인. **PASS**.
...
**실기기 결과(2026-09-12) — FAIL 발견**: 식사 활동 블록만 생기고 왕복 이동(특히 복귀 편)이 생기지 않았다. **원인**: `MealScheduleSheet.applyDestinationSuggestion`이 "다음 일정"이 있을 때만 복귀지(`destination`)를 채우고, 없으면(가장 흔한 경우) `nil`로 비워둬 "돌아가는 이동 만들기" 토글이 켜져 있어도 도착지가 없어 조용히 이동이 안 만들어졌다. **수정**: 다음 일정이 없으면 출발지(`origin`)로 되돌아가는 것을 기본값으로 채우도록 `FullSirView.swift`를 수정(2026-09-12). 같은 증상이 AI `create_activity`(갈 때만 생기고 올 때 안 생김)에도 있었는데, 이쪽은 모델이 `return_to_query`를 빠뜨리는 문제라 툴 설명에 "왕복이면 반드시 채워라"를 강조하는 프롬프트 보강으로 완화(모델 순응 여부는 다음 실기기 확인에서 재검증 필요 — 100% 해결 보장 아님).
추가로, **AI가 만든 식사 일정이 `meals.json`에 기록되지 않는 문제**도 함께 발견됐다 — `executeCreateActivity`가 활동+이동만 만들고 `addMeal`을 호출하지 않았음(REQ-014의 취지상 AI 경로도 포함돼야 함). **수정**: `create_activity`에 `log_as_meal` 인자 추가, true면 `FullSirView.save()`와 동일하게 `addMeal(activityId:)`까지 호출.
수정 후 iOS·macOS 무경고 빌드 확인 + 아이폰 실기기 설치·실행 완료(2026-09-12). **다음에 폰을 만질 때 재확인 필요**: (a) FullSirView에서 "돌아가는 이동 만들기" 켠 채 저장 시 왕복 이동 둘 다 생기는지, (b) AI 채팅으로 "그거로 일정 잡아줘" → 최근 먹은 것 목록에 남는지, (c) AI가 왕복을 만들 때 `return_to_query`를 실제로 채우는지(프롬프트 보강 효과 검증). **PASS-WITH-FOLLOWUP**(코드 수정 완료·빌드 확인 완료, 수정 후 라이브 재확인만 남음).
...
**미실행**: AC-204의 수정으로 `activityId`가 이제 정상적으로 채워지므로 근본 전제는 해소됐지만, 삭제 시나리오 자체(미래/과거 분기)는 이번 세션에서 실기기로 확인하지 않았다 — 다음 실기기 확인 목록에 포함.
...
**실기기 결과(2026-09-12)**: "이상 없음" — 빈 상태 UI 정상. **PASS**.
...
**실기기 결과(2026-09-12)**: "위치권한 꺼도 잘 작동" — 정상 동작 확인. **PASS**.

### 추가 반영 — Day 12에서 나온 신규 요구사항 2건 (원래 AC 범위 밖, 사용자 승인 후 구현)

기존 REQ/AC 어디에도 없던 두 가지가 실기기 테스트 중 새로 발견됐다. 버그가 아니라 설계 결정이라 AskUserQuestion으로 확인 후 반영했다(둘 다 "지금 추가" 선택):

1. **목적지 주변 섹션 위치 이전** — AC-201 위 "실기기 결과" 참고. `EventDetailView`(이동 일정)에서 `ActivityDetailView`(활동 일정)로 이전. `nearbySection`/`nearbyRow`/`loadNearby` 로직을 그대로 옮기고, 기준 좌표만 `event.destination`에서 `activity.location`으로 교체.
2. **be full sir 기준 위치 직접 검색** — `FullSirView.Basis`에 `.custom` 케이스 추가 + `PlaceField`(기존 컴포넌트 재사용) 기반 자유 검색. 현재 위치·오늘 일정 목적지 둘 다 기준 삼기 어려운 외곽 지역 대응.

두 항목 모두 iOS·macOS 무경고 빌드 확인 완료, 아이폰 실기기 설치까지 완료. **라이브 재확인 필요**: 활동 상세를 열었을 때 섹션이 뜨는지, 이동 상세에서는 빠졌는지, "직접 검색"으로 낯선 지역 검색이 되는지.
...
- [ ] Day 8(M1) 조사 결론 도출 — Implementation Kickoff Approval 전 필수 전제 (여전히 미해결, 유일하게 남은 항목)
- [x] AC-101~116 전부 확인(Day 11) — 전부 PASS(AC-105는 정적 검사 한계로 측정 불가 표시)
- [x] AC-201~207 전부 확인(Day 12) — AC-204에서 버그 2건 발견, 그 자리(Day 13)에서 수정 완료. AC-205는 다음 실기기 확인 대기
- [x] Day 13에서 발견 사항 전부 수정(왕복 이동 미생성, meals.json 미기록 2건) — §7(리스크)로 이월한 항목 없음
- [ ] 회귀(be on-time sir) 이상 없음 확인 — 이번 세션에서 be full sir 범위만 실기기 확인, be on-time sir 핵심 흐름 회귀 확인은 다음 실기기 확인 목록에 포함
- [ ] `STATUS.md`에 be full sir MVP 완료 반영 — STATUS.md는 2026-06-30 기준으로 낡아 있어 별도 갱신 필요(사업계획서용 요약이라 우선순위 낮음, CLAUDE.md 참고)
