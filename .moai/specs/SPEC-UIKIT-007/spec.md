...
| 0.1.3 | 2026-09-24 | **sync 단계 종료 — 3-phase close(`in-progress → implemented → completed`, 한 커밋). 본문은 고치지 않았다**(frontmatter와 이 행만). ① AC-008 (1): 루트 `plan.md` 후속 17을 수리 `b73013b`·유입 `1b98e14`와 함께 닫았다. ② AC-008 (2): 줄 수가 645 그대로라 옮긴 인용은 0이다. 대조는 했다. 후속 17의 서로 다른 끝점 15개 중 13개가 `73ceb43`과 바이트가 같고, 다른 둘(`:166`·`:536`)은 수리 자체다. 후속 14는 4개 모두 같다. 대조 중 이전 오인용 하나를 고쳤다 — 후속 5의 `AddEventView:601·:604`는 t3 트리(`d5203cb`)의 좌표였는데, t6(`02481c6`)의 +26을 따라가지 못해 `store.addEvent(…)`를 가리키고 있었다 → `:627·:630`(`cmp` exit 0). `AddActivityView` 인용 7끝점(후속 11·14·16)이 t5의 한 줄 삭제로 밀린 것은 재기만 하고 리드에게 넘겼다 — 이 카드가 만진 파일이 아니다. ③ AC-008 (3): `CHECKLIST.md` 무변경, `AddEventView` 줄번호 인용 0. ④ 게이트는 이 레인이 직접 돌렸다: 새 DerivedData로 iOS·macOS 전체 빌드 `BUILD SUCCEEDED`, swift 경고 0/0(`SwiftCompile` 42·38), 프록시 7/7. 드라이버는 돌리지 않았다(§0). ⑤ 독립 `--deep` 렌즈(`code-safety`, 다른 손): 결함 0 · 관찰 7. 이 수리가 깨운 셋(값이 정해진 출발지 줄의 측위 스피너 · 권한 거부 뒤 "현재 위치" 탭이 저장된 출발지를 비움 · 새 주석의 전칭 범위)은 루트 `plan.md` 후속 19로, 원래 있던 넷(`recomputeEstimates` 경합 · 저장 버튼의 `saving` 무시 — 중복 저장 가설 · 빈 이름 칩 · 씨앗 인자 두 벌)은 후속 20으로 올렸다. REQ-006은 위반이 아니라고 판정했다("~로 끝나지 않고"는 결과가 하나 더 있다는 뜻으로 읽힌다). **기록 결손**: AC-005·006의 "값 기록" 절은 충족되지 않았다 — run §E.2 M3c에 수치가 없다. 판정은 유지하고 수용 여부는 리드 몫이다(`progress.md` §E.4.4). **인용 기준 부기**: 이 SPEC 문서 셋의 루트 `plan.md` 인용 `:530`(후속 14)·`:533`(후속 17)은 `73ceb43` 좌표다. run이 t7 행을 넣어 지금은 `:531`·`:534`다. 기록은 `progress.md` §E.4 |
...
- 따라서 카드 본문의 "c5396b3 이전부터 존재(이번 Day 회귀 아님)"는 앞부분만 참이다 — **Phase 1.7 안의 t2가 만든 회귀다.** 루트 `plan.md:539`의 "t6가 만든 것 아님"은 참이다.
