...
| 0.2.2 | 2026-09-15 | 착수 시 사용자가 확정한 읽기 3건을 본문에 명시(progress.md §E.2 "착수 시 확정한 설계 결정 2건 + 승인 형태와 달라진 점 1건"의 전사, 줄번호는 HEAD `b9ef208` 실측). ① REQ-005 — 출발지 부재도 문자 그대로 카드로 흐른다: `resolveOrigin`의 '집' → 현재 위치 폴백은 `orDefault:`(`:1980`)로 가두고 `check_travel_time`(`:1791`)에서만 켠다("칩 요청" 표현도 카드로 정정). ② REQ-011 — 카드 대상은 생성 도구 전반: `create_activity`도 오가는 이동을 만들 때만 행이 생기고(머무는 활동은 제외), 왕복은 가는 편·오는 편 수단을 따로 묻는다. ③ REQ-014 — 이동수단(`TransportMode`) 행에는 `[직접입력]`을 붙이지 않는다: 세 칩이 전체 집합이며 §4 승인 그림과 일치(구현 근거 `:544`) |
...
- **REQ-005 (Unwanted)**: `resolvedMode`/`resolveOrigin` shall not substitute stored preference values for absent arguments, and `applyStatedPreferences(from:)` shall not exist — 부재는 부재로 관측되어 REQ-011의 카드 요청으로 흐른다. 근거(변경 전 실측): `resolvedMode` `:757`, `resolveOrigin` `:1810`, `applyStatedPreferences` 정의 `:321`·호출 `:279`. **빈 문자열을 부재로 읽는 처리(`:1095-1096` 주석의 모양)는 유지한다** — 제거하는 것은 저장값 대체뿐이다. **출발지 부재도 문자 그대로 카드로 흐른다(2026-09-15 착수 시 확정, progress.md §E.2)** — `resolveOrigin`의 조용한 즐겨찾기 '집' → 현재 위치 폴백은 `orDefault:` 파라미터(정의 `:1980`, 가드 `:1991`) 뒤로 가뒀고, 켜는 곳은 `check_travel_time` 처리부(`:1791`) 하나뿐이다(등록이 아니라 조회라 되물을 이유가 없다). 생성 도구에 `origin_query`가 없으면 조용한 대체가 아니라 REQ-011의 카드 행이 생긴다.
...
- **REQ-014 (Where capability gate)**: **Where** an argument is asked on the card, its row shall offer a `[직접입력]` escape — 칩은 모든 값을 열거할 수 없는 경우의 탈출구이므로 원칙적으로 모든 행에 붙는다(§4 승인 형태의 `[직접]`). **예외 — 이동수단(`TransportMode`) 행에는 붙이지 않는다**(2026-09-15 착수 시 확정, progress.md §E.2): 세 칩이 곧 전체 집합이라 열거하지 못하는 값이 존재하지 않고, §4의 승인된 그림도 그 줄에는 `[직접]`이 없다 — 직접입력이 없어도 고를 수 없는 값이 생기지 않음을 확인했다(구현 근거 `Shared/AIAssistant.swift:544`). `도착 여유` 행은 **진짜 `0분` 칩**을 제공한다: 0은 "여유 없이"라는 정당한 요청이지만, 문장 파서의 `Self.minutes`는 `v > 0`만 돌려주고 생성 경로 수용부도 `$0 > 0`만 통과시켜(`Shared/AIAssistant.swift:1336-1337`·`:1341`) **발화로는 진짜 0을 말할 수 없다** — 이 프로젝트가 받아들인 트레이드오프였고, 이 SPEC의 카드가 그 받아들여진 격차를 닫는다: 0이 다시 설정 가능해진다. 빈 직접입력으로는 제출되지 않는다.
