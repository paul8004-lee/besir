...
- 커밋: `ebd841c`(M1 게이트) · `02481c6`(M2 코드)·`27d35a5`(M2 문서) · `84328e3`(M3 코드)·
  `1727585`(M3 문서) · `42c29c0`(M4 문서) · **본 커밋**(sync 종결). 코드 커밋은 `02481c6`·
  `84328e3` 둘뿐이고 나머지는 문서다. 본 커밋의 sha를 여기 적지 않는 것은 §E.3과 같은 이유다 —
  커밋은 자기 sha를 담을 수 없고, amend로 적으면 그 순간 다시 어긋난다(이 sync가 한 번 겪었다).
