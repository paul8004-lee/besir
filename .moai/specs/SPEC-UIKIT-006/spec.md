...
| 0.1.2 | 2026-09-23 | **sync 단계 종료 — 3-phase close(`in-progress → implemented → completed`, 한 커밋).** ① REQ-020 (b): `CHECKLIST.md`의 `Store`·`GoogleCalendarService`·`ContentView` 인용 끝점 **89개**를 옛 줄(`00ab661`)과 새 줄의 본문 바이트 대조로 옮겼다(75 이동·14 제자리). 지운 세 함수를 가리키던 인용은 **0건**이었다. 1차 기계 귀속이 **열다섯을 틀렸다** — `Store` 뒤에 붙은 `AIAssistant` 인용 아홉을 `Store`로, `ContentView` 인용 여섯(재확인 블록에 `.swift` 없이 적은 다섯 + K9의 `span(for:on:)`)을 다른 파일로 보았다. 앞 심볼을 옛 트리 전 파일에서 찾아 귀속을 정한 뒤 옮겼다. ② L3 오인용의 대상은 `Store` 동기화 2-4단계(알림 해제 루프)다 — t1 sync(`c156c4d`)가 `:1455-1459`를 `AIAssistant`로 보고 −95 밀어 `:1360-1364`가 됐다 → `Store.swift:1427-1431`. 대조 중 같은 부류 셋(I2 `:788`→`:871` · A3 · L14 — 뒤의 둘은 t6의 `AIAssistant` 이동을 따라가지 못함)도 함께 고쳤다. ③ 루트 `plan.md:105`의 `deleteExpired` 항목을 닫고(D-3 (a)) §Phase 1.7에 t5 행을 넣었다. ④ 게이트: iOS·macOS를 **새 DerivedData로 전체 빌드**해 `BUILD SUCCEEDED`·swift 경고 0/0(`build/` 증분 빌드는 컴파일 단계가 0이라 경고 증거가 못 됐다), 프록시 7/7. 드라이버는 **다시 돌리지 않았다** — 컴파일 집합 12파일이 run 레인이 잰 `b8bbe1c`와 바이트 동일해(`git diff --quiet` exit 0) 같은 입력을 다시 재는 일이었고, 한 번 돌 때마다 실제 앱 데이터와 키체인을 건드린다. 205/205는 run 레인 측정의 인용이다. ⑤ 독립 `--deep` 렌즈(code-safety, 다른 손): 결함 0. **동작 검증은 해당 없음** — 바뀐 동작이 0곳이라 실기기 전용 항목이 없다(§3.1 머리말). 기록은 `progress.md` §E.4 |
