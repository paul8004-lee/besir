...
- **줄 수는 Tier S(300 미만)에 한참 못 미치지만, 파일 수는 Tier M의 범위다.** 주 체크아웃의
  ...
  과장한다. 새 타입·새 화면·새 저장 경로·설계 분기가 하나도 없다. 운영자가 착수 승인 게이트에서
  이 판단을 확정했다(2026-09-23, §2 항목 5).
...
| M | REQ | AC | 요약 | 상태 |
|---|---|---|---|---|
| M1 | — (D-1~D-3) | — | **착수 승인 게이트** — 세 결정 · `CLAUDE.md` 수정 확인 · Tier 확인(§2). 고른 안이 M2의 내용과 §4의 렌즈 명단을 정한다 | ✅ (a)(a)(a) · `CLAUDE.md` 승인 · Tier S — 2026-09-23 |
...
컴패니언 레인은 게이트 결정을 운영자에게 묻지 않는다(예외 하나는 항목 4 — 프로젝트 지시 파일 편집의
세션 안 동의). 리드가 아래 다섯 항목을 착수 승인 게이트에서 제시했고,
**운영자가 다섯 항목 모두 권고안으로 확정했다**(2026-09-23, 리드가 plan 레인에 전달). 각 안의 결과·비용·
전제 확인은 `spec.md` §4에 실측과 함께 있다. 이 plan 세션은 운영자의 답을 직접 보지 않았고,
리드의 전달로 기록한다.

1. **D-1 해소 — (a) 제거.** `LocationManager.openLocationSettings()`(`:118-129`)와 그것만 쓰던
   `AppKit`/`UIKit` 조건부 import(`:3-7`)를 지운다(REQ-010·AC-003). 연결안 (b)와 유지안 (c)는 채택하지 않았다.
2. **D-2 해소 — (a) 유지 + 이유 주석 한 줄.** `Store.updateMeal(_:)`을 남기고, SPEC-FULL-001 REQ-003을
   가리키는 한국어 주석 한 줄을 바로 위에 단다(REQ-011·AC-004).
3. **D-3 해소 — (a) 제거.** `Store.deleteExpired()`(`:1318-1333`)를 지운다(REQ-012·AC-005). sync에서
   루트 `plan.md:105`의 "알고도 안 고친 것" 항목을 닫는다(REQ-020).
4. **`CLAUDE.md:127` 수정 승인.** `[SettingsView.swift:103](Shared/SettingsView.swift#L103)` →
   `[SettingsView.swift:71](Shared/SettingsView.swift#L71)`. 리드는 운영자가 직접 확인했다고 전했다.
   **다만 `CLAUDE.md`는 프로젝트 지시 파일이고 이 확인은 다른 세션을 거쳐 전달됐다.** 그래서 run 레인은
   M4에서 이 줄을 고치기 **직전에** 자기 세션에서 운영자에게 한 번 더 확인받는다. 확인으로 치는 수단은
   **운영자가 run 세션에 직접 입력한 확인** 또는 **그 편집의 권한 프롬프트 승인** 둘뿐이다. 리드나 다른
   세션을 거친 메시지는 확인이 아니고, run 레인은 이 확인을 위해 리드에게 블로커 보고로 되묻지 않는다.
   수단과 결과는 `progress.md` §E.2에 적는다(AC-006 (1)). 리드나 다른 세션의 디스패치만으로는 편집 권한이
   생기지 않는다는 규칙은 그대로다. 확인이 거절되면 REQ-020 (a)와
   AC-006 (1)이 빠지고 run 파일은 다섯이 된다.
5. **Tier S 확정.** run 파일은 6개다. `spec-workflow.md:140-141`의 표에서 5~15개는 Tier M의 파일
   범위이지만, 운영자가 줄 수(삭제 49줄)와 설계 내용 0을 근거로 S를 확정했다. `acceptance.md`는 두지 않는다.
...
위 표에서 채택된 것은 **(a) 행 셋**이다. 표는 결정 기록으로 남긴다.

**게이트를 통과했다(2026-09-23).** 이제 run 디스패치를 받으면 착수할 수 있다. 세 결정이 모두 같은
`Store.swift`·`LocationManager.swift`를 고치므로, M2와 M3은 한 레인에서 §1의 순서대로 차례로 한다.
