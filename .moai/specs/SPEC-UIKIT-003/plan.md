...
- **REQ 14건 · AC 10건** — 실측 명령과 함께: `grep -c '^- \*\*REQ-' spec.md` = **14**,
  `grep -c '^## AC-' acceptance.md` = **10**. Tier M 상한(REQ 16 / AC 16)은 SPEC-UIKIT-002
...
| M | REQ | AC | 요약 | 상태 |
|---|---|---|---|---|
| M1 | — (D-1·D-2·D-3) | — | 설계 확정 — SPEC 3종 완결. **D-3(이동 다리의 여유)만 미해소로 남는다** — M3 시작 전 운영자 결정, 기본값은 "올리지 않음" | 🟡 |
| M2 | REQ-001~003 | AC-001·AC-002 | 컴포넌트 표면 — `anchored` 옵션(해석·글자·확정 네 경로), 시각 줄 두 줄 배치 + 기준 칩 출발→도착, 흐림 판정이 기준 있는 줄을 본다. **전부 기본값·내부 변경**이라 `AIAssistant`·`AddEventView`·`AIChatView`는 한 줄도 안 바뀐다 | ⬜ |
| M3 | REQ-010~013 | AC-003·AC-004 | `AddActivityView` 전환 — `@State` 12개 → `card` 하나, 다리 둘을 줄 멤버십으로, 알림 리드 줄 신설(요청 ②), header/footer는 카드 밖 유지 | ⬜ |
...
- **D-3(이동 다리의 여유)이 M3 착수 전 운영자 결정거리다.** 기본값은 "올리지 않음"(요청 ②가
  알림만 들었다)이고, 그러면 캡션에 "여유 10분"만 남는다. 운영자가 "올림"을 고르면 REQ가 하나
  늘고(15건, 상한 이내) **REQ-003이 선행 조건이 된다**(§1.4의 휴면 결함이 그 줄과 함께 깨어난다).
  결정을 run 세션이 조용히 내리지 않는다.
