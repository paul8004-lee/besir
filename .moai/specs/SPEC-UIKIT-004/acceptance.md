...
**판정 (sync 종결, 2026-09-20 — progress.md §E.3 AC-009 기록이 근거).** 시뮬레이터 10항목 중
**9항목 통과**(S1~S3·S5~S10 — S10은 code-safety 노트③의 최대 Dynamic Type 줄바꿈 확인까지 포함).
**S4(단계 옆 "오후 3:05")만 이월**: 시뮬레이터에서 대중교통 경로가 계산되지 않아 단계행 자체가
렌더링되지 않았다(사용자 관찰) — `shortTimeFmt` 코드·주석 무변경은 AC-006 6절 대조와 grep
(EV `DateFormatter()`=1·주석 그대로)으로 확보했으므로, 렌더 결과 확인만 Day 닫기 실기기 목록으로
넘긴다. 실기기 전용(VoiceOver 3종·카드 질감)도 같은 이월 목록에 있다.
