...
| 0.2.2 | 2026-09-18 | **sync 단계 종료 — 3-phase close(`in-progress` → `completed`).** ① 기계 게이트 4종을 sync lane이 HEAD 소스에서 **다시** 실측했다(드라이버 205/205 · iOS·macOS `BUILD SUCCEEDED` 소스 경고 0 · 프록시 7/7). 재실행한 이유는 결과 불신이 아니라 **귀속**이다 — 워크트리의 `build-ios.log`·`build-mac.log`(10:04)가 마지막 소스 커밋 `bb09e01`(10:09)보다 앞서 찍혀 HEAD에 귀속할 수 없었다. ② 독립 렌즈(`code-safety`, `--security --deep`)가 "순수 추출" 주장의 반증을 시도하고 실패했다 — 정규화 diff 22줄 전수 설명, 값↔참조·캡처·기본인자·옵셔널·액터 격리·프로퍼티 래퍼·부수효과 순서 변화 0건, 위해 4종 신규 0건, 보안 0건. ③ **원본으로 되돌린 2건**: `EditCard.swift:14`의 `whenFormatter`가 원본 `private static`에서 `internal`로 넓어져 있던 것(호출부 1곳, `private` 한 단어로 복구)과 `:47` 주석의 `주체`→`주제` 오타(REQ-011 의미 보존). ④ **루트 `CHECKLIST.md` 코드 근거 179건 수리** — 이 카드가 만든 드리프트다. `AIAssistant.swift` 인용 168건이 −95·−117로 밀렸고 AI 카드 뷰 11건은 `EditCardView.swift`로 주소가 옮겨졌다. 옛 줄과 새 줄의 본문을 **바이트 단위로 전수 대조**해 불일치 0건을 확인했고, 다른 파일을 가리키던 30건은 보호했다(1차 자동 분류가 그중 9건을 오귀속했고 감사에서 잡았다). `spec.md §5`가 CHECKLIST를 "확인만"으로 잡은 근거는 "**신규 항목**이 없다"였는데, 기존 항목의 줄번호가 깨지는 것은 그 범위가 보지 못한 별개 사건이었다. ⑤ 하네스 상태 파일 2건(`.moai/state/`)을 버전 관리에서 제거하고 `.gitignore`에 규칙을 넣었다. ⑥ AC 매트릭스를 닫았다 — 7건 ✅, AC-007은 ⚠️ 13/14(시뮬레이터, ⑮ VoiceOver 이월). 근거·미검증·잔여 위험은 `progress.md §E.4` |
