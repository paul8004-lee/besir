...
| 선택 표시 | 체크 글리프 + 글자 굵기(색 단독 아님) | `:434-437` |
| 칩 낭독 | `accessibilityElement(children: .contain)` + 줄 이름·사유 결합 라벨 | `:271-275` |
| 흐린 여유 줄 | 투명도 0.5 + 캡션("출발 기준이라 쓰지 않아요") 이중 표현 | `:262-268` |
| 확인 버튼 | 잠김 사유가 `accessibilityHint`로 읽힘 | `:554` |
| 칩 높이 | iOS 44 / macOS 28, 양쪽 `@ScaledMetric(relativeTo: .callout)` | `:174-179` |
| 디바운스 | 뷰에 타이머가 없다 — 묶음은 어댑터 쪽 | `:489-490` |
...
- **Given** `accepts`가 종류별 상한을 갖고(여유 `0...Store.maxBufferMinutes`, 알림 `0...1440`, 반복 `1...Store.maxRecurrenceWeeks`, 이동수단 `TransportMode(rawValue:)`, 시각은 접두 + 정규 ISO — `:88-111`), 장소 줄이 자유 텍스트 확정 버튼을 두지 않으며, 칩이 가로 스크롤 없이 줄바꿈으로 흐르고
- **When** 추출이 끝나면
- **Then**
  1. 상한이 **상수 참조로** 남는다 — 숫자가 문구나 코드에 박히지 않는다. 거절 문구는 여전히 범위를 적지 않는다("그 값은 쓸 수 없어요", `:254`).
  2. 장소 줄에 자유 텍스트를 그대로 확정하는 버튼이 **없다** — 후보 탭만이 확정 경로다(`:391-396`, 결함 P). 0건/오프라인을 구분한 척하지 않는 문구가 그대로다(`:412-415`).
  3. `ChipFlow`의 `arrange`가 크기 계산과 배치에 **같은 함수**로 쓰인다(`:604`) — 따로 세면 높이와 실제 줄 수가 어긋난다. 가로 `ScrollView`가 없다.
...
- 함께 확인: `AddActivityView.PlaceField`(`:187-267`)의 계약 6 위반(`.quaternary`·`.secondary`·`.bordered`)과 디바운스 부재가 **그대로 남아 있다** — 고쳤다면 범위를 넘은 것이다(t3 소관). 이 AC는 "안 고친 것"을 통과로 판정하는 유일한 AC다.
