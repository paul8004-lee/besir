...
| t13 | 수정 경로 조용한 실패(C4) — `Store.updateActivity`의 인라인 `try? createEvent` 재등록을 **업로드 큐 경유로**(pending/failed 신호·성공 gid는 큐가 단일 출처 — updateEvent·updateRecurringSeries와 같은 문턱 셋: `googleConnected`·gid 저장 전 소거·`autoAddToCalendar`) + code-safety 권고 1건 반영(sync 2-5 성공 시 pending 미해제 "올리는 중" 영구 좌초 — `Store.swift:1460-1462` 한 줄+주석) | 없음(원인 확정 — plan 생략; 근거: day-close-20260924 §6 row 7·§7) | **run 완료 — 병합 대기(2026-09-25).** 코드 `3b42e45`. 게이트 4종 이 레인 재실측(한 줄 반영 뒤 최종 트리): 드라이버 **212/212·exit 0·불변식 9/9·실데이터 바이트 대조 통과** · iOS·macOS 신선 DerivedData 전체 빌드 `BUILD SUCCEEDED` **swift 경고 0/0**(42/38 컴파일 위상 — Store.swift 컴파일 확인) · proxy 7/7. code-safety 4부류 **신규 결함 0**(권고 1은 반영, 신규 관찰 1·상속 잔여 5는 progress §5 기록 — 후속 카드 후보 셋). 문서: CHECKLIST 인용 29건 재사상(+9, reconcile 끝점 +12)·결함 4번 처리됨 표기·맨몸 좌표 1건·plan 후속 17·20② 좌표 수리(20②는 t9 트리 좌표로 이미 어긋났던 사전 오인용)·progress.md 커밋 포함 |
