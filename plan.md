...
| t14 | Theme·계약 6 정리(C5) — CB 3건(AddEventView :763·:766·:771, t13 재사상 좌표로 확인) + 같은 부류(ActivityDetailView 5줄·AddActivityView:574 = PlaceField radius 후속 3) + 후속 16(AIChatView 말풍선 직접 색) + 후속 4(돋보기 접근성 라벨) + 신규(장소 검색·날짜·시각 고르기 칩 점선→실선 — chip() dashed 파라미터 제거·점선 서술 주석 4건 갱신). 디스패치 계약 문구 "색은 전부 Theme 토큰 경유"에 따라 스윕으로 드러난 같은 부류 13곳도 함께 정리(치환 실측 `.secondary` 16·`.tertiary` 2·직접 색 4·literal 반경 3 — 최초 보고의 17·3·5·10은 오기) | 없음(원인 확정 — plan 생략; 근거: day-close-20260924 §6 rows 12·17·§7·운영자 실기기 확인 세션) | **완료 — 병합 대기(2026-09-25).** 게이트 이 레인 재실측: 드라이버 **217/217·exit 0·실데이터 바이트 대조 통과** · proxy 7/7 · iOS·macOS 무경고. code-safety **PASS**(7렌즈 전부 구동·FIX-NOW 0·권고 3 — a11y 아이콘 부류·ShareExtension:19 별도 카드 후보). 문서화 예외 2건 유지(지도 클립 12·노선색 뱃지 white). 코드 `96529a4` + 문서 `35b24e2` + **sync FAIL(D1~D6) 수리(2026-09-25)**: D1 후속 14 좌표 6+grep 4수·D2 후속 3·4·5·16 닫힘 표기·D3 계수 정정(커밋 메시지의 17·3·5는 오기)·D5 SPEC 두 자리 대체 표기·D6 progress 인용 정정·CHECKLIST G16 `:186-254` + D4(a) 출발 마커 `.tint(Theme.travel)`·A1 쌍둥이 돋보기 라벨 — 코드 `df827b7`, 신선 dd 재게이트 iOS 42·macOS 38 위상 무경고. 수리 문서 `1eceb43`. 확인 목록은 progress §5(⑦~⑩ 추가) |
| t15 | 문구·주석 묶음(C6) — 후속 18①②(주석 과적용 — 보임새 표면 둘 명시·사전 천장)·19③(자리 나열 제거)·B5 "이동 수단"→"이동수단" 표기 4곳·B7 계산 후 불가 수단 고지(모드 줄 노트)·D-3 고지(폼 시각 줄 노트 + 상세 `alarmStatus` 네 갈래 — 꺼짐/예약됨/과거 거부/64건 창 밖 대기) | 없음(Class B — SPEC 없음; 근거: day-close-20260924 §7 C6·t2/t6/t7 원형) | **sync FAIL(D1·D2) 수리 완료 — 병합 대기(2026-09-25).** 본체 `876fc34`(게이트는 sync가 격리 사본으로 독립 재실측 — 드라이버 217/217·프록시 7/7·iOS·macOS 무경고) + 수리 `aea15d4`: D1 알림 토글이 시각 줄 노트를 다시 조립(`syncFieldExtras()` 호출 — 켤 때 경고 미표시·끌 때 잔류를 동시에 닫음)·D2 폼 문안 "알림 시각" 정정(상세 어휘에 맞춤)·A2 상세 정보 행 "받지 않음" 조건부(리드 수용). code-safety 심사 PASS(차단 0 — D1 양방향 경로 추적·`NotificationManager:34`와 경계 여집합 대조). 문서 수리(같은 날): 살아있는 인용 1차 18+2좌표·2차 맨몸 87좌표(범위 뒤끝 포함)·행별 판정 7좌표 재사상(잔여 0 — 원장 문자열 대조+양성 대조)·후속 18 닫힘·후속 19 ③ 잔여 닫힘·progress 좌표 6건·D6 정정(CLAUDE.md 레시피는 주 체크아웃 master 판에만 낡음 — 브랜치 트리는 t1 `f9cd9bb`부터 정확). 확인 목록은 판정문 §5·progress §5(운영자 몫 — D1·D2 기기 재현 포함) |
...
3. `PlaceField` cornerRadius 8 → `Theme.radius`(:501 계열). **닫혔다 — t14(`96529a4`, 2026-09-25).** 선택 박스 배경이 `Theme.radius`로 갔다(현 좌표 `AddActivityView:574`).
4. `PlaceField` 돋보기 버튼에 접근성 라벨이 없다(:518) — VoiceOver에 이름 없는 버튼으로 읽힌다. **닫혔다 — t14(`96529a4`, 2026-09-25).** `.accessibilityLabel("장소 검색")`이 붙었다(현 좌표 `AddActivityView:596`).
5. `ConflictBanner` 잔여 계약 6 위반과 radius(`AddEventView`:627·:630 — 처음 적은 `:601·:604`는 t3 트리 `d5203cb`의 좌표였는데, t6 `02481c6`이 파일을 26줄 민 뒤 옮기지 않아 `store.addEvent(…)` 호출을 가리키고 있었다. 2026-09-24 t7 sync가 두 줄을 `cmp`로 대조해 옮겼다). **닫혔다 — t14(`96529a4`, 2026-09-25).** 겹침 배너의 `.secondary`·`.tertiary`·반경 8이 muted·faint·`Theme.radius`로 갔다(현 좌표 `AddEventView:732·:735·:740` — 항목 머리의 `:627·:630`은 그날 기록).
