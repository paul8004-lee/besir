...
| t7 | 일정 편집의 출발지 덮어쓰기 수리 — 출발지 줄에 편집 씨앗 한 줄(`chosen: editing?.origin?.name`) + `confirmedPlace` 주석 재서술(후속 17의 수리, 결함은 t2 `1b98e14`에서 유입) | `SPEC-UIKIT-007` | **run 종결 — 2026-09-23.** 수리 커밋 `b73013b`(draft→in-progress), AC-001~003 기계 신호 통과, AC-004 iOS·macOS 새 DerivedData 무경고 초록. 렌즈(`ui-design`·`code-safety`) 지적 0건 — D14(편집 중 출발지 검색 기준 좌표)는 결함 아님으로 양 렌즈 일치. 시뮬레이터 한 자리(dd-a·dd): 파트 A **(가) 재현**(AC-005) → 파트 B 전 단계 통과(AC-006·007 — 8번 동일 시간 관측은 그대로 기록) + SPEC-UIKIT-005 AC-009 보류 판정 통과·6번 통과, SPEC-UIKIT-003 AC-010 15/16(16번 VoiceOver 시뮬레이터 무반응 — 실기기 이월). 수치(N₀·X·D 등)는 운영자 별도 기록 없이 판정만 수령 — 갭은 progress.md §E.2에 명시. 후속 17 닫기·인용 재정렬은 sync 몫(AC-008). 관측된 카드 밖 사안(AC-009 7·9번·U-3·U-4)은 Day 닫기 이월 목록에서 리드 관리 |
