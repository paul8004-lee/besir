...
final class AIAssistant: ObservableObject {
    ...
    /// 열화의 방향이 안전한 쪽이다. 보임새 변화는 한 표면이 아니다 — 칩 글자가 구분 문자열로
    /// 길어지는 것과, 확인 요약 말풍선에 같은 문자열이 찍히는 것(resolvePendingAsk가 chosenLabel로
    /// 줄을 세우고, 옵션에 없는 확정 값은 customLabel이 값을 그대로 돌려준다)이다.
    ...
    ///
    /// 사전이 자라는 차원은 이름의 수가 아니라 **골라 본 서로 다른 지점의 수**다 — 같은 지점은
    /// 열쇠를 늘리지 않으므로, 사전 크기의 천장은 이 대화에서 확정한 지점 수다.
    ///
